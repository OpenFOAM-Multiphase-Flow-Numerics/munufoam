...
)
{
    ...
    const DynamicField<label>& interfaceLabels = oldSurf.interfaceLabels;

    // Loop through cells
    forAll(interfaceLabels, i)
    {
        const label celli = interfaceLabels[i];
        if (mag(oldSurf.normal[celli]) != 0)
        {
            const scalar dt = deltaT(rDeltaT,celli);
            ...
            // Cell is cut
            const point x0 = oldSurf.centre[celli];
            vector n0 = -oldSurf.normal[celli];
            n0 /= (mag(n0));
...
)
{
    ...
    const geometricVoF& oldSurf = geoVoF_.surface(timeState::oldState);
    ...
    timeIntegratedFlux(rDeltaT,oldSurf);
